...
const buttonVariants = cva(
  "inline-flex items-center justify-center rounded-md text-sm font-semibold transition-colors focus:outline-none  dark:hover:bg-slate-800 dark:hover:text-slate-100 disabled:opacity-50 dark:focus:ring-slate-400 disabled:pointer-events-none dark:focus:ring-offset-slate-900 data-[state=open]:bg-slate-100 dark:data-[state=open]:bg-slate-800",
  {
    variants: {
      variant: {
        default:
          "bg-primaryColor text-white hover:opacity-90 dark:bg-slate-50 dark:text-slate-900",
        black: "bg-black text-white hover:bg-slate-900 dark:hover:bg-black-600",
        outline:
      ...
      size: {
        default: "h-[36px] rounded-full py-2 px-4",
        sm: "h-9 px-4 rounded-full",
        lg: "h-[52px] px-[32px] rounded-full",
      },
    },
    defaultVariants: {
      variant: "default",
